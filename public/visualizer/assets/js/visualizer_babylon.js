...
function load_scene(elementId) {
    ...
    this.load_room = function (room_id, data, onLoadFunc) {
        ...
        data = data || {};
        load_room_id = data.preset ? ( data.preset_room_id || room_id ) : room_id;
        ...
        data.collider_size = data.collider_size || 1;
        loadp.data = data;
        ...
        loadp.camera.inputs.attached.touch.touchAngularSensibility = 2000;
        ...
        loadp.camera.speed = loadp.data.camera_speed || 0.01
        camera.invertRotation = true;
        showLoader();
        console.log("remove!");
        console.log('camerainverted')
        //set up the room collider to avoid going out of the room
        loadp.camera.ellipsoid = new BABYLON.Vector3(data.collider_size, data.collider_size, data.collider_size);   // this is required for the collider to work
    ...
    this.doMaterial = function(k, tk, m) {
        ...
        if ( tk.dispose ) {
            ...
            if (tk.alpha) {
                ...
                m.material.alpha = typeof( tk.alpha ) == 'number' ? tk.alpha : 0.1;
                if ( tk.reflectionTexture ) {
                    let reflectionTexture = new BABYLON.CubeTexture.CreateFromPrefilteredData(
                        tk.reflectionTexture, loadp.scene
                    );
                    m.material.reflectionTexture = reflectionTexture
                }
            }
            if ( tk.emissiveColor ) {
                //m.material.emissiveColor = BABYLON.Color3(tk.emissiveColor[0], tk.emissiveColor[1], tk.emissiveColor[2])
            }
    ...
    this.pickMesh = function( hitFunc, notHitFunc ) {
        ...
        if (pp1.hit) {
            let pp = pp1.pickedPoint;
            console.debug(`Picked Mesh: {x: ${pp._x}, y: ${pp._y}, z: ${pp._z}}`);
            pp = pp1.pickedMesh.name;
            console.debug('Picked', pp);
    ...
    this.getSprite = function(name) {
        let l = loadp.spriteManager.sprites.filter(function(n) { return n.name == 'sprite_' + name})
        if ( l.length ) {
            return l[0]
        }
        return
    }
    this.getScreenCoordinates = function(vector) {
        if ( vector instanceof Array && vector.length == 3 ) {
            vector = new BABYLON.Vector3(...vector)
        }
        if ( ! vector instanceof BABYLON.Vector3 ) {
            logger.error("Input vector is not in the correct format")
            return [null, null]
        }
        ...
        BABYLON.Vector3.ProjectToRef(
            ...
            loadp.scene.getTransformMatrix(), //transform matrix
            { x: 0, y: 0, width: loadp.canvas.width, height: loadp.canvas.height }, //viewport
            pos
        )
        return [loadp.canvas.getClientRects()[0].x + pos.x, loadp.canvas.getClientRects()[0].y + pos.y]
    }
    this.showInteractive = function() {
        ...
        loadp.showSprites();
        if ( window.scene_interactive && typeof ( window.scene_interactive ) == 'function' ) {
            window.scene_interactive();
        }
...
window.initFunction = async function(elementId, room_id, params, onLoadFunc) {
    params = params || {};
    room_id = room_id || 'bathroom_large';
...
BABYLON.DefaultLoadingScreen.prototype.hideLoadingUI = function(){
    ...
    setTimeout(function() {
        $("#leftcont").removeClass("restrict");
        $("#resize").removeClass("restrict");

        console.log("restrict remove");
    }, 3000);
}
