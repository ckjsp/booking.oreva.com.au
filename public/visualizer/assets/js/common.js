let opened_filters = [];
let opened_modal = [];
let dave_url = "https://test.iamdave.ai";
if ( window.location.origin.indexOf('iamdave.ai') >= 0 ) {
    dave_url = "https://test.iamdave.ai";
} else if ( window.location.origin.indexOf('localhost') >= 0 ) {
    dave_url = "https://test.iamdave.ai";
} else if ( window.location.origin.indexOf('hindware.com') >= 0 ){
    dave_url = "https://dashboard.iamdave.ai";
}
let enterprise_id = "hindware";
let signup_api_key = "d1e881a1-186a-3a43-9133-bf7047971e6b";
// device detection
let is_iPad = navigator.userAgent.match(/Mac OS/i) != null;
let is_iPhone = navigator.userAgent.match(/iPhone/i) != null;
...
function signup(data, postfunc, errorFunc) {
    ...
    let customer_id = params.customer_id || getCookie("customer_id") || data.customer_id;
    let ddata = {
       "name": "anonymous",
       "customer_id": customer_id,
       "email": "ananth+" + generate_random_string(8) + "@iamdave.ai",
       "validated": true,
...
function callback_submit(name, contact_number, message, callbackFunc, errorFunc){
    ajaxRequestWithData(
        dave_url + "/object/customer/" + getCookie("customer_id"),
        "PATCH",
...
function quick_enquiry_submit(name, email_id, contact_number, pincode, interested_products, message, callbackFunc, errorFunc){
ajaxRequestWithData(
    dave_url + "/object/customer/" + getCookie("customer_id"),
    "PATCH",
    getCookie("authentication"),
    JSON.stringify({
          ...
          "phone_number": contact_number,
          "pincode": pincode,
          "interested_product": interested_products,
          "query": message
    }),
    function(data){
        if ( callbackFunc && typeof(callbackFunc) == 'function') {
